<template>
  ...
  <Form.Item v-bind="{ ...schemaItem.formItemProps }" :label="renderLabelHelpMessage" :name="schemaItem.field"
    :labelCol="itemLabelWidthProp.labelCol" :wrapperCol="itemLabelWidthProp.wrapperCol" :rules="getRules">
    <slot v-if="schemaItem.slot" :name="schemaItem.slot" v-bind="getValues"></slot>
    <component v-else :is="getComponent" :ref="setItemRef" :key="schemaItem.field" :allowClear="true"
      v-on="componentEvents" v-model:[modelValueType]="modelValue[schemaItem.field]" v-bind="getComponentProps">
...
import { cloneDeep, isFunction, isNull, isString } from 'lodash'
...
const getComponentProps = computed(() => {
  ...
  if (isFunction(componentProps)) {
    ...
    compProps.placeholder ??= isString(component) ? createPlaceholderMessage(component, label) : undefined

    return compProps
  }
  if (component !== 'RangePicker' && isString(component)) {
    ; (componentProps as any).placeholder ??= createPlaceholderMessage(component, label)
  }
...
const getRules = computed(() => {
  const { rules: defRules = [], component, rulesMessageJoinLabel, label, dynamicRules, required } = props.schemaItem
